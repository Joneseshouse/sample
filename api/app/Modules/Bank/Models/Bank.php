...
class Bank extends Model{
	...
	public static $fieldDescriptions = [
		...
		'branch' => 'str,required|max:250',
		'account_number' => 'str,required|max:50',
		'type' => 'str,required|max:5',
	];

	public static $searchFields = ['title', 'branch'];

	public static function list($params=[], $keyword=null, $orderBy='-id'){
		try{
			$orderBy = Tools::parseOrderBy($orderBy);
			$listItem = Bank::where($params);
	...
	public static function addItem($input){
		try{
			...
			$checkDuplicate = self::where(["account_number" => $input["account_number"]])->first();
			if($checkDuplicate){
				return ResTools::err(
	...
	public static function removeItem($id, $userId, $force=false){
		try{
			$result = null;
			$listId = explode(',', $id);
