...
class Shop extends Model{
	...
	public static $searchFields = ['uid', 'title', 'vendor'];

	public function orderItems(){
		return $this->hasMany('App\Modules\OrderItem\Models\OrderItem', 'shop_id');
	}

	public function purchases(){
		return $this->hasMany('App\Modules\Purchase\Models\Purchase', 'shop_id');
	}

	public static function list($params=[], $keyword=null, $orderBy='-id'){
		try{
			$orderBy = Tools::parseOrderBy($orderBy);
			$listItem = Shop::where($params);
	...
	public static function addItem($input){
		try{
			...
			if(!array_key_exists('uid', $input) || !$input['uid']){
				$input['uid'] = $input['title'];
			}
		...
		try{
			...
			if($listItem->count()){
				foreach ($listItem as $item) {
					if($item->uid === '000'){
						return ResTools::err(
							trans('messages.can_not_remove_shop'),
							ResTools::$ERROR_CODES['BAD_REQUEST']
						);
					}
					if($item->orderItems->count() || $item->purchases->count()){
						return ResTools::err(
							trans('messages.can_not_remove_in_used_shop'),
							ResTools::$ERROR_CODES['BAD_REQUEST']
						);
					}
					if($force){
