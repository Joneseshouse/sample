...
class Article extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			bulkRemove: true,
			params: {},
			dataLoaded: store.getState().articleReducer.list.length?true:false
	    };
	...
	setInitData(initData){
		this.props.articleAction('newList', {list: [...initData.data.items], pages: initData.data._meta.last_page});
		this.setState({dataLoaded: true});
	}

	componentDidMount(){
		document.title = "Articles";
		// if(!this.props.articleReducer.list.length){
			if(window.initData){
		...
		switch(state){
			case 'mainModal':
				...
				if(id && open){
					...
					this.props.articleAction('obj', Tools.getInitData(labels.mainForm));
					this.setState(newState);
				}
			break;
		}
	...
	handleCheck(id, checked){
		...
		this.props.articleAction('edit', {checked}, index);
	}

	handleCheckAll(){
		let list = this.props.articleReducer.list;
		if(filter(list, {checked: true}).length === list.length){
			this.props.articleAction('uncheckAll');
		}else{
			this.props.articleAction('checkAll');
		}
	}
	...
	handleRemove(id=null){
		...
		if(id === null){
			let listId = [];
			this.props.articleReducer.list.map(value => {
				if(value.checked){
		...
		Tools.apiCall(apiUrls.remove, {id}).then((result) => {
	    	if(result.success){
	    		...
	    		let listIndex = listId.map(id => {
					return store.getState().articleReducer.list.findIndex(x => x.id === parseInt(id));
	    		});
				this.props.articleAction('remove', null, listIndex);
	    	}
	    });
	}
			...
			return Tools.apiCall(apiUrls[id?'edit':'add'], id?{...params, id}:params).then((result) => {
		    	if(result.success){
		    		...
		    		if(id){
						...
						this.props.articleAction('add', data);
		    		}
		    		dispatch(reset('ArticleMainForm'));
		    		this.toggleModal(null, 'mainModal', false);
		    		Tools.goToUrl('article/detail', [result.data.id]);
		    	}else{
