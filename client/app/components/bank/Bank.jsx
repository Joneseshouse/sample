...
class Bank extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			bulkRemove: true,
			params: {},
			dataLoaded: store.getState().bankReducer.list.length?true:false
	    };
	...
	setInitData(initData){
		this.props.bankAction('newList', {list: [...initData.data.items], pages: initData.data._meta.last_page});
		this.setState({dataLoaded: true});
	}

	componentDidMount(){
		document.title = 'Bank';
		if(!this.props.bankReducer.list.length){
			if(window.initData){
		...
		switch(state){
			case 'mainModal':
				...
				if(id && open){
					...
					this.props.bankAction('obj', {...Tools.getInitData(labels.mainForm), region: store.getState().bankReducer.defaultRegion});
					this.setState(newState);
				}
			break;
		}
	...
	handleCheck(id, checked){
		let index = store.getState().bankReducer.list.findIndex(x => x.id===id);
		this.props.bankAction('edit', {checked}, index);
	}

	handleCheckAll(){
		let list = this.props.bankReducer.list;
		if(filter(list, {checked: true}).length === list.length){
			this.props.bankAction('uncheckAll');
		}else{
			this.props.bankAction('checkAll');
		}
	}
		...
		try{
			...
			return Tools.apiCall(apiUrls[id?'edit':'add'], id?{...params, id}:params).then((result) => {
		    	if(result.success){
		    		...
		    		if(id){
						let index = store.getState().bankReducer.list.findIndex(x => x.id === id);
						this.props.bankAction('edit', data, index);
		    		}else{
						this.props.bankAction('add', data);
		    		}
		    		dispatch(reset('BankMainForm'));
	...
	handleRemove(id=null){
		...
		if(id === null){
			let listId = [];
			this.props.bankReducer.list.map(value => {
				if(value.checked){
		...
		Tools.apiCall(apiUrls.remove, {id}).then((result) => {
	    	if(result.success){
	    		...
	    		let listIndex = listId.map(id => {
					return store.getState().bankReducer.list.findIndex(x => x.id === parseInt(id));
	    		});
				this.props.bankAction('remove', null, listIndex);
	    	}
	    });
	}
