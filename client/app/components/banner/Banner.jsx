...
class Banner extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			bulkRemove: true,
			params: {},
			dataLoaded: store.getState().bannerReducer.list.length?true:false
	    };
	...
	setInitData(initData){
		this.props.bannerAction('newList', {list: [...initData.data.items], pages: initData.data._meta.last_page});
		this.setState({dataLoaded: true});
	}

	componentDidMount(){
		document.title = 'Banner';
		if(!this.props.bannerReducer.list.length){
			if(window.initData){
		...
		switch(state){
			case 'mainModal':
				...
				if(id && open){
					...
					this.props.bannerAction('obj', Tools.getInitData(labels.mainForm));
					this.setState(newState);
				}
			break;
		}
	...
	handleCheck(id, checked){
		let index = store.getState().bannerReducer.list.findIndex(x => x.id===id);
		this.props.bannerAction('edit', {checked}, index);
	}

	handleCheckAll(){
		let list = this.props.bannerReducer.list;
		if(filter(list, {checked: true}).length === list.length){
			this.props.bannerAction('uncheckAll');
		}else{
			this.props.bannerAction('checkAll');
		}
	}
	...
	handleRemove(id=null){
		...
		if(id === null){
			let listId = [];
			this.props.bannerReducer.list.map(value => {
				if(value.checked){
		...
		Tools.apiCall(apiUrls.remove, {id}).then((result) => {
	    	if(result.success){
	    		...
	    		let listIndex = listId.map(id => {
					return store.getState().bannerReducer.list.findIndex(x => x.id === parseInt(id));
	    		});
				this.props.bannerAction('remove', null, listIndex);
	    	}
	    });
	}
	...
	handleChange(eventData, dispatch){
		try{
			const params = {...eventData, category_id: this.props.params.category_id};
			...
			return Tools.apiCall(apiUrls[id?'edit':'add'], id?{...params, id}:params).then((result) => {
		    	if(result.success){
		    		...
		    		if(id){
						let index = store.getState().bannerReducer.list.findIndex(x => x.id===id);
						this.props.bannerAction('edit', data, index);
		    		}else{
						this.props.bannerAction('add', data);
		    		}
		    		dispatch(reset('BannerMainForm'));
