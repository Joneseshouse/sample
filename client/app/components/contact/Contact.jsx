...
class Contact extends React.Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			bulkRemove: true,
			params: {},
			dataLoaded: store.getState().contactReducer.list.length?true:false
	    };
	...
	setInitData(initData){
		this.props.contactAction('newList', {list: [...initData.data.items], pages: initData.data._meta.last_page});
		this.setState({dataLoaded: true});
	}

	componentDidMount(){
		document.title = 'Contact';
		if(!this.props.contactReducer.list.length){
			if(window.initData){
		...
		switch(state){
			case 'mainModal':
				...
				if(id && open){
					...
					this.props.contactAction('obj', Tools.getInitData(labels.mainForm));
					this.setState(newState);
				}
			break;
		}
	...
	handleCheck(id, checked){
		let index = store.getState().contactReducer.list.findIndex(x => x.id===id);
		this.props.contactAction('edit', {checked}, index);
	}

	handleCheckAll(){
		let list = this.props.contactReducer.list;
		if(filter(list, {checked: true}).length === list.length){
			this.props.contactAction('uncheckAll');
		}else{
			this.props.contactAction('checkAll');
		}
	}
	...
	handleRemove(id=null){
		...
		if(id === null){
			let listId = [];
			this.props.contactReducer.list.map(value => {
				if(value.checked){
		...
		Tools.apiCall(apiUrls.remove, {id}).then((result) => {
	    	if(result.success){
	    		...
	    		let listIndex = listId.map(id => {
					return store.getState().contactReducer.list.findIndex(x => x.id === parseInt(id));
	    		});
				this.props.contactAction('remove', null, listIndex);
	    	}
	    });
	}
		...
		try{
			...
			return Tools.apiCall(apiUrls[id?'edit':'add'], id?{...params, id}:params).then((result) => {
		    	if(result.success){
		    		...
		    		if(id){
						let index = store.getState().contactReducer.list.findIndex(x => x.id===id);
						this.props.contactAction('edit', data, index);
		    		}else{
						this.props.contactAction('add', data);
		    		}
		    		dispatch(reset('ContactMainForm'));
