...
import {labels} from '../../cnBillOfLandingFail/_data';
...
class FailTable extends React.Component {
	...
	shouldComponentUpdate(newProps, newState) {
		const sameList = Tools.isSameCollection(
			this.props.exportBillDetailReducer.listFail,
			newProps.exportBillDetailReducer.listFail
		);
		if(sameList){
	...
	_renderRightTool(value, row, index){
		return (
			<TableRightTool
				...
				toggleModal={() => this.props.toggleModal(row.id, 'failModal')}
		...
		return (
			<div>
				<Table
					...
					data={this.props.exportBillDetailReducer.listFail}
					/>
			</div>
		);
	}
