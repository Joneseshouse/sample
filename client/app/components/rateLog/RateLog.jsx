...
import RateLogLayout from './RateLog.layout';

class RateLog extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			mainModal: false,
			...
			bulkRemove: true,
			params: {},
			dataLoaded: store.getState().rateLogReducer.list.length?true:false
	    };
	...
	setInitData(initData){
		this.props.rateLogAction('newList', {list: [...initData.data.items], pages: initData.data._meta.last_page});
		this.setState({dataLoaded: true});
	}

	componentDidMount(){
		document.title = 'RateLog';
		if(!this.props.rateLogReducer.list.length){
			if(window.initData){
		...
		switch(state){
			case 'mainModal':
				...
				if(id && open){
					...
					this.props.rateLogAction('obj', Tools.getInitData(labels.mainForm));
					this.setState(newState);
				}
			break;
		}
	...
	handleCheck(id, checked){
		let index = store.getState().rateLogReducer.list.findIndex(x => x.id===id);
		this.props.rateLogAction('edit', {checked}, index);
	}

	handleCheckAll(){
		let list = this.props.rateLogReducer.list;
		if(filter(list, {checked: true}).length === list.length){
			this.props.rateLogAction('uncheckAll');
		}else{
			this.props.rateLogAction('checkAll');
		}
	}
	...
	handleRemove(id=null){
		...
		if(id === null){
			let listId = [];
			this.props.rateLogReducer.list.map(value => {
				if(value.checked){
		...
		Tools.apiCall(apiUrls.remove, {id}).then((result) => {
	    	if(result.success){
	    		...
	    		let listIndex = listId.map(id => {
					return store.getState().rateLogReducer.list.findIndex(x => x.id === parseInt(id));
	    		});
				this.props.rateLogAction('remove', null, listIndex);
	    	}
	    });
	}
		...
		try{
			...
			return Tools.apiCall(apiUrls[id?'edit':'add'], id?{...params, id}:params).then((result) => {
		    	if(result.success){
		    		...
		    		if(id){
						let index = store.getState().rateLogReducer.list.findIndex(x => x.id===id);
						this.props.rateLogAction('edit', data, index);
		    		}else{
						this.props.rateLogAction('add', data);
		    		}
		    		dispatch(reset('RateLogMainForm'));
