...
import RoleTypeLayout from './RoleType.layout';
...
class RoleType extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			bulkRemove: true,
			params: {},
			dataLoaded: store.getState().roleTypeReducer.list.length?true:false
	    };
	...
	setInitData(initData){
		this.props.roleTypeAction('newList', {list: [...initData.data.items], pages: initData.data._meta.last_page});
		this.setState({dataLoaded: true});
	}

	componentDidMount(){
		document.title = "Phân quyền";
		if(!this.props.roleTypeReducer.list.length){
			if(window.initData){
		...
		switch(state){
			case 'mainModal':
				...
				if(id && open){
					...
					this.props.roleTypeAction('obj', Tools.getInitData(labels.mainForm));
					this.setState(newState);
				}
			break;
		}
	...
	handleCheck(id, checked){
		let index = store.getState().roleTypeReducer.list.findIndex(x => x.id===id);
		this.props.roleTypeAction('edit', {checked}, index);
	}

	handleCheckAll(){
		let list = this.props.roleTypeReducer.list;
		if(filter(list, {checked: true}).length === list.length){
			this.props.roleTypeAction('uncheckAll');
		}else{
			this.props.roleTypeAction('checkAll');
		}
	}
	...
	handleRemove(id=null){
		...
		if(id === null){
			let listId = [];
			this.props.roleTypeReducer.list.map(value => {
				if(value.checked){
		...
		Tools.apiCall(apiUrls.remove, {id}).then((result) => {
	    	if(result.success){
	    		...
	    		let listIndex = listId.map(id => {
					return store.getState().roleTypeReducer.list.findIndex(x => x.id === parseInt(id));
	    		});
				this.props.roleTypeAction('remove', null, listIndex);
	    	}
	    });
	}
		...
		try{
			...
			return Tools.apiCall(apiUrls[id?'edit':'add'], id?{...params, id}:params).then((result) => {
		    	if(result.success){
		    		...
		    		if(id){
						let index = store.getState().roleTypeReducer.list.findIndex(x => x.id===id);
						this.props.roleTypeAction('edit', data, index);
		    		}else{
						this.props.roleTypeAction('add', data);
		    		}
		    		dispatch(reset('RoleTypeMainForm'));
