...
class MainTable extends React.Component {
	...
	shouldComponentUpdate(newProps, newState) {
		const sameList = Tools.isSameCollection(this.props.userReducer.list, newProps.userReducer.list);
		if(sameList){
	...
	render() {
		...
		forEach(headingData, (value, key) => {
			let data = {
				...
				dataIndex: key,
				key,
				render: (value, row, index) => Tools.tableData(labels.mainForm, row, key, this.props.params, store.getState().userReducer)
			}
			if(value.width){
		...
		return (
			<div>
				<Table
					...
					data={this.props.userReducer.list}
					/>
				<div className="pagination-wrapper">
					<Paginator
						pageCount={this.props.userReducer.pages}
